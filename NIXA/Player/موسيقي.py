...
@Client.on_message(command2(["تحميل","حمل"]) & ~filters.edited)
def song(_, message):
...
@Client.on_message(
    command2(["حمل_فيديو","تحميل_فيديو","حمل_فيد","تحميل_فيد"]) & ~filters.edited
)
...
async def lyrics(_, message):
    try:
        if len(message.command) < 2:
            await message.reply_text("» **هاذي كلمات الاغنيه.**")
            return
